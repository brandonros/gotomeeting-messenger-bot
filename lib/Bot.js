...
Bot.prototype.handleBotMessage = async function(from, body) {
  ...
  if (matchingCommand.parameters.length) {
    ...
    responses = await matchingCommand.fn(self, fromChannelId, parametersMap);
  }

  else {
    responses = await matchingCommand.fn(self, fromChannelId, body);
  }
...
Bot.prototype.sendMessage = function(to, body) {
  ...
  this.send(`<message from='${this.userId}/${this.clientName}' msgid='${msgId}' to='${to}' type='chat' xmlns='jabber:client'><body>**[BOT]:** ${body}</body><origin-id xmlns='urn:xmpp:sid:0' id='${msgId}'/></message>`);

  return msgId;
};

Bot.prototype.getMessageHistory = function(withUserId, numMessages, before) {
  var msgId = uuid();
  var queryId = uuid();

  this.send(`<iq type='set' xmlns='jabber:client' id='${msgId}:sendIQ'>
    <query xmlns='urn:xmpp:mam:0' queryid='${queryId}'>
      <x xmlns='jabber:x:data' type='submit'>
        ...
        <field var='with'>
          <value>${withUserId}@chat.platform.getgo.com</value>
        </field>
      </x>
